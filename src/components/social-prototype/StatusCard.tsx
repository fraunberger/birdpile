...
export function StatusCard({ status, profile, onClickProfile, isOwn = false, isAdmin = false, currentUserId = null, onEdit, showPostReportButton = true, disableItemEditing = false }: StatusCardProps) {
    ...
    const { user } = useAuth();
    const { deleteStatus, moveStatus, getPostedDates, addComment, deleteComment, reportStatus, reportComment, softDeleteStatus, softDeleteComment, removeItemFromActive, addItemToStatus } = useSocialStore();
    ...
    const handleMovePost = async () => {
        if (!moveDate || moveSubmitting) return;
        const postedDates = getPostedDates();
        if (postedDates.includes(moveDate)) {
            pushToast({ message: 'A post already exists on that date.', tone: 'error' });
            return;
        }
        setMoveSubmitting(true);
        try {
            await moveStatus(status.id, moveDate);
            pushToast({ message: `Post moved to ${moveDate}.`, tone: 'success' });
            setShowMovePicker(false);
            setMoveDate('');
        } catch (error) {
            pushToast({ message: error instanceof Error ? error.message : 'Failed to move post', tone: 'error' });
        } finally {
    ...
    const openMovePicker = () => {
        ...
        setMoveDate('');
        // On mobile, trigger the native date picker after render
        setTimeout(() => {
            moveDateRef.current?.showPicker?.();
            moveDateRef.current?.focus();
        }, 100);
    };
    ...
    return (
        <div className="border border-neutral-200 bg-white px-3 py-2.5 font-mono">
            ...
            {showMovePicker && (
                <div className="mb-2 p-2 border border-neutral-200 bg-neutral-50 flex items-center gap-2 flex-wrap">
                    <span className="text-[10px] uppercase tracking-widest text-neutral-500">Move to:</span>
                    <input
                        ref={moveDateRef}
                        type="date"
                        value={moveDate}
                        onChange={(e) => setMoveDate(e.target.value)}
                        className="border border-neutral-300 px-2 py-1.5 text-xs font-mono outline-none focus:border-neutral-500 min-w-[140px]"
                        style={{ fontSize: '16px' }}
                    />
                    <button
                    ...
                    >
                        {moveSubmitting ? 'Moving...' : 'Confirm'}
                    </button>
                    <button
                        onClick={() => { setShowMovePicker(false); setMoveDate(''); }}
                        className="px-2 py-1.5 text-[10px] uppercase tracking-widest text-neutral-400 hover:text-neutral-700"
                    >
                        Cancel
                    </button>
                </div>
