...
export async function POST(req: NextRequest) {
  try {
    ...
    if (action === "social.status.move") {
      const statusId = String(payload.statusId || "");
      const newDate = String(payload.newDate || "");
      if (!statusId || !newDate) {
        return NextResponse.json({ error: "Missing statusId or newDate" }, { status: 400 });
      }
      ...
      // Check that the target date doesn't already have a post
      const { data: conflict } = await supabaseAdmin
        ...
        .maybeSingle();
      if (conflict) {
        return NextResponse.json({ error: "A post already exists on that date" }, { status: 409 });
      }
