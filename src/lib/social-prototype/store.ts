...
class SocialStore {
    ...
    async moveStatus(statusId: string, newDate: string) {
        await socialWrite('social.status.move', { statusId, newDate });
        await this.fetchStatuses();
    }

    getPostedDates(): string[] {
        return this.state.statuses.map(s => s.date);
    }
...
export function useSocialStore() {
    ...
    return {
        ...
        moveStatus: (id: string, newDate: string) => socialStore.moveStatus(id, newDate),
        getPostedDates: () => socialStore.getPostedDates(),
